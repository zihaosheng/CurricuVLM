...
class Logger():
    def __init__(self, log_dir="./logs", exp_name=None, env_name=None, seed=0, config=None, filename='evaluator.csv',
        ...
        if debug:
            ...
            self.logger = csv.DictWriter(self.csv_file, fieldnames=(
    ...
    def update(self, score, total_steps):
        ...
        print(lu.colorize(
            f"\nTime: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}, Time spent from previous logger: {(current_log_time - self.previous_log_time):.3f} s",
            'yellow', bold=True))
        ...
        if not self.debug:
            epinfo = {"mean_score": avg_score, "total_steps": total_steps, "std_score": std_score,
...
# class SafeLogger():
#     def __init__(self, log_dir="./logs", exp_name=None, env_name=None, seed=0, config=None, mode='train',
#                  filename="logger.csv", fieldnames=[], debug=False):
#         self.exp_name = exp_name
#         self.env_name = env_name
#         self.seed = seed
#         self.previous_log_time = time.time()
...
class SafeLogger():
    def __init__(self, log_dir="./logs", exp_name=None, env_name=None, seed=0, config=None, fieldnames=[], debug=False,
                 filename="logger.csv"):
        self.exp_name = exp_name
        self.env_name = env_name
        self.seed = seed
        ...
        self.train_filename = '{}-{}'.format('train', filename)
        self.eval_filename = '{}-{}'.format('eval', filename)
        ...
        if debug:
            ...
            self.train_csv_file = open(self.log_dir + '/' + self.train_filename, 'w', encoding='utf8')
            self.eval_csv_file = open(self.log_dir + '/' + self.eval_filename, 'w', encoding='utf8')

            self.train_logger = csv.DictWriter(self.train_csv_file, fieldnames=(self.fieldnames))
            self.eval_logger = csv.DictWriter(self.eval_csv_file, fieldnames=(self.fieldnames))

            self.train_logger.writeheader()
            self.eval_logger.writeheader()
    ...
    def update(self, fieldvalues, total_steps, mode='train'):
        epinfo = {}
        fieldvalues = [total_steps] + fieldvalues
        ...
        if not self.debug:
            for filedname, filedvalue in zip(self.fieldnames, fieldvalues):
                epinfo[filedname] = filedvalue

            if mode == 'train':
                self.train_logger.writerow(epinfo)
                self.train_csv_file.flush()
                idx = len(fieldvalues) // 2 + 1
            elif mode == 'eval':
                self.eval_logger.writerow(epinfo)
                self.eval_csv_file.flush()
                idx = len(fieldvalues)

            for i in range(1, idx):
                self.writer.add_scalar("{}/{}".format(mode, self.fieldnames[i]), fieldvalues[i], fieldvalues[0])
