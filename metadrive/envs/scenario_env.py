...
class ScenarioEnv(BaseEnv):
    ...
    def reward_function(self, vehicle_id: str):
        ...
        long_now, lateral_now = current_lane.local_coordinates(vehicle.position)  # 看了源码,
        ...
        if self.config["use_lateral_reward"]:  # 如果使用的话，远离车道中心会使得奖励减小
            lateral_factor = clip(1 - 2 * abs(lateral_now) / 6, 0.0, 1.0)  # +-0.5
        else:
            lateral_factor = 1.0

        reward = 0
        reward += self.config["driving_reward"] * (long_now - long_last) * lateral_factor  # dense reward
