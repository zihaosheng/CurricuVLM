...
from imitation_env import ImitationMetaDriveEnv
...
if __name__ == "__main__":
    ...
    env = ImitationMetaDriveEnv(config=config_train, action_space_type='continuous')
    torch.manual_seed(args.seed)
    np.random.seed(args.seed)
    rng = np.random.default_rng(args.seed)

    logger = SafeLogger(exp_name='imitation-SQIL', env_name=args.env, seed=args.seed,
    ...
    for i in range(100):
        # airl_trainer.train(250_000)
        sqil_trainer.train(total_timesteps=10_000)
        sqil_trainer.rl_algo.save("{}sac-{}".format(logger.log_dir, i))
        sqil_trainer.rl_algo.save_replay_buffer("{}sac-buffer".format(logger.log_dir))
        env.close()
        ...
        eval_episode_overtake_adv = eval_policy(sqil_trainer.policy, eval_env, adv_generator, record_t=i)
        ...
        eval_env.close()
        env = ImitationMetaDriveEnv(config=config_train)
        venv = DummyVecEnv([lambda: RolloutInfoWrapper(env)])
        sqil_trainer = sqil.SQIL(
            venv=venv,
            demonstrations=transitions,
        ...
        sqil_trainer.rl_algo.load("{}sac-{}".format(logger.log_dir, i))
        sqil_trainer.rl_algo.load_replay_buffer("{}sac-buffer".format(logger.log_dir))
