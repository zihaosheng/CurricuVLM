...
def eval_policy(policy, eval_env, adv_generator, eval_episodes=100):
    ...
    for ep_num in range(eval_episodes):
        ...
        while not done:
            ...
            action, buffer_action = policy.select_action(obs=state.reshape(1, -1), learning_starts=args.start_timesteps,
                                          action_noise=policy.action_noise, n_envs=1)
...
if __name__ == "__main__":
    ...
    file_name = "{}-SAC".format(args.mode)
    logger = SafeLogger(exp_name="{}-SAC".format(args.mode), env_name=args.env, seed=args.seed,
    ...
    # ########################################
    policy = SAC('MlpPolicy', env, verbose=0, seed=0, device='cuda', learning_starts=10000)
    ...
    for t in range(int(args.total_timesteps)):
        ...
        action, buffer_action = policy.select_action(obs=new_obs.reshape(1, -1), learning_starts=args.start_timesteps,
                                                     action_noise=policy.action_noise, n_envs=1)  # [1, 2]
        ...
        policy._store_transition(policy.replay_buffer, buffer_action, new_obs.reshape(1, -1), np.array([reward]), np.array([done]), [info])
        ...
        if policy.num_timesteps > 0 and policy.num_timesteps > policy.learning_starts:
            ...
            policy.train(batch_size=policy.batch_size, gradient_steps=policy.gradient_steps)
        # ##############################################################################
        ...
        # ##############################################################################
            ...
            if t - last_eval_step > args.eval_freq:
                ...
                if args.save_model: policy.save(os.path.join(logger.log_dir, "{}-{}".format(file_name, t)))

            # Reset environment
            new_obs, done = env.reset(), False
            adv_generator.before_episode(env)
