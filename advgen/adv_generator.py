...
class AdvGenerator():
    ...
    def after_episode(self, update_AV_traj=False, mode='train', is_safe=False):
        if update_AV_traj:
            ...
            if is_safe: seed = self.env.last_seed
            else: seed = self.env.current_seed
            if mode == 'train':
                self.storage[seed]['AV_trajs'].append(latest_ego_traj)
                self.storage[seed]['AV_probs'].append(self.ego_prob)
            elif mode == 'eval':
                self.storage[seed]['AV_trajs_eval'].append(latest_ego_traj)
            else:
    ...
    def _parse(self, use_adv_future=False):
        ...
        for i, track_id in enumerate(tracks_ids):
            ...
            if i == 1:
                ...
                if use_adv_future: adv_gt_future = track_data['state']['position'][11:, :2]
        ...
        if use_adv_future:
            return ego_navigation_route, adv_gt_future
    ...
    def generate(self, mode='train'):
        ...
        trajs_OV = pred_trajectory[1]  # [0] should be prediction for ego, shape [32, 80, 2]

        probs_OV = pred_score[1]  # shape [32,]
        ...
        for j, prob_OV in enumerate(probs_OV):
            P1 = prob_OV
            traj_OV = trajs_OV[j][::5]  # shape [16, 2]
            yaw_OV = get_polyline_yaw(trajs_OV[j])[::5].reshape(-1, 1)  # shape [16, 1]
            ...
            for i, prob_AV in enumerate(probs_AV):
                P2 = prob_AV
                traj_AV = trajs_AV[i][::5]  # downsample to speed up
