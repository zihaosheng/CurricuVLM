...
def is_on_side(ego_pos, ego_yaw, adv_pos, side='left', angle_threshold=np.pi / 4):
    """
    :param ego_pos: Position of the ego vehicle (x, y)
    :param ego_yaw: Yaw of the ego vehicle (in radians)
    :param adv_pos: Position of the adversary vehicle (x, y)
    :param side: A string specifying which side to check (left, right, behind, or front).
    :param angle_threshold: Maximum angle difference (in radians) to consider the adversary as being on the specified side.
    :return: True if the adversary is on the specified side within the given angle threshold, False otherwise
    """
    direction = np.array([np.cos(ego_yaw), np.sin(ego_yaw)])
    relative_pos = adv_pos - ego_pos

    if side == 'left':
        side_direction = np.array([-direction[1], direction[0]])
    elif side == 'right':
        side_direction = np.array([direction[1], -direction[0]])
    elif side == 'behind':
    ...
    if relative_pos_norm < 1e-2:
        return False

    relative_pos_normalized = relative_pos / relative_pos_norm
    dot_product = np.clip(np.dot(side_direction, relative_pos_normalized), -1.0, 1.0)
    angle = np.arccos(dot_product)

    return angle < angle_threshold


def check_on_side(ego_traj, ego_yaw, adv_traj, side='left', consecutive_threshold=5):
    """
    :param ego_traj: Numpy array [t, 2] where each entry is a (x, y) position.
    :param ego_yaw: Numpy array [t,] the yaw (heading angle in radians).
    :param adv_traj: Numpy array [t, 2] where each entry is a (x, y) position.
    :param side: A string specifying which side to check ('left' or 'right').
    :param consecutive_threshold: minimum number of consecutive frames where the adversary vehicle is on the left.
    :return:
    """
    side_count = 0
    max_side_count = 0
    ...
    for i in range(len(ego_traj)):
        if is_on_side(ego_traj[i], ego_yaw[i], adv_traj[i], side=side):
            side_count += 1
            max_side_count = max(max_side_count, side_count)
        else:
            side_count = 0

    return max_side_count >= consecutive_threshold, max_side_count


def find_adv(scenario_data, default_agent, adv_agent, ego_traj, ego_yaw, side='left'):
    min_dist = dict()
    corspd_ego_yaw, corspd_ego_traj, corspd_veh_traj = dict(), dict(), dict()
    min_ego_yaw, min_ego_traj, min_veh_traj = None, None, None
    for k, v in scenario_data['tracks'].items():
        if k in ['sdc', default_agent]: continue
        if scenario_data['metadata']['object_summary'][k]['type'] != 'VEHICLE' or \
            scenario_data['metadata']['object_summary'][k]['distance'] < 5: continue
        veh_traj = v['state']['position'][:, :2]
        if np.all(veh_traj[:11] == 0): continue
        min_length = min(len(ego_traj), len(veh_traj))
        if k == adv_agent:
            distances = np.linalg.norm(ego_traj[:min_length] - veh_traj[:min_length], axis=1)
            min_index = np.argmin(distances)
            min_ego_yaw, min_ego_traj, min_veh_traj = ego_yaw[min_index], ego_traj[min_index], veh_traj[min_index]
        is_on_side, _ = check_on_side(ego_traj[:min_length], ego_yaw[:min_length], veh_traj[:min_length], side=side)
        if is_on_side:
            distances = np.linalg.norm(ego_traj[:min_length] - veh_traj[:min_length], axis=1)
            # min_dist[k] = np.min(distances)
            min_index = np.argmin(distances)
            min_dist[k] = distances[min_index]

            corspd_ego_yaw[k] = ego_yaw[min_index]
            corspd_ego_traj[k] = ego_traj[min_index]
            corspd_veh_traj[k] = veh_traj[min_index]

    if min_dist and adv_agent not in min_dist:
        adv_agent = min(min_dist, key=min_dist.get)
        min_ego_yaw, min_ego_traj, min_veh_traj = corspd_ego_yaw[adv_agent], corspd_ego_traj[adv_agent], \
                                                  corspd_veh_traj[adv_agent]
    return adv_agent, min_ego_yaw, min_ego_traj, min_veh_traj


def select_adv_agent(critical_area, scenario_data, default_agent, adv_agent):
    ego_traj = scenario_data['tracks'][default_agent]['state']['position'][:, :2]
    ego_yaw = get_polyline_yaw(ego_traj)
    if "behind" == critical_area or "front" == critical_area:
        return find_adv(scenario_data, default_agent, adv_agent, ego_traj, ego_yaw, side=critical_area)
    elif "left" in critical_area:
        return find_adv(scenario_data, default_agent, adv_agent, ego_traj, ego_yaw, side='left')
    elif "right" in critical_area:
        return find_adv(scenario_data, default_agent, adv_agent, ego_traj, ego_yaw, side='right')
    else:
        return adv_agent, None, None, None
...
class AdvGenerator():
    def __init__(self, parser):
        ...
        self.collision_figs = deque(maxlen=20)

    def before_episode(self, env, mode='train'):
        self.env = env
        self.ego_traj = []
        self.ego_prob = 1.
        self.adv_traj = []
        ...
        if not self.storage.get(self.env.current_seed):
            traffic_motion_feat, adv_agent, ego_navigation_route, adv_past = self._parse(mode=mode)
    ...
    def log_SV_history(self):
        try:
            sv_objs = self.env.engine.get_objects()
            for id, sv in sv_objs.items():
                if id == 'default_agent': continue
                if id in self.sv_pos:
                    self.sv_pos[id].append(sv.position)
                    self.sv_vel[id].append(sv.velocity)
                    self.sv_heading[id].append(sv.heading_theta)
                else:
                    self.sv_pos[id] = [sv.position]
                    self.sv_vel[id] = [sv.velocity]
                    self.sv_heading[id] = [sv.heading_theta]
        except:
            pass

    def _parse(self, mode='train'):
        ...
        self.gpt_select_veh = dict()
        if self.gpt_response and mode == 'train':
            try:
                gpt_response = json.loads(self.gpt_response)
                critical_area = gpt_response['CriticalArea'].lower()
            except:
                critical_area = self.gpt_response.split('CriticalArea')[-1].lower()
            adv_agent, min_ego_yaw, min_ego_traj, min_veh_traj = select_adv_agent(critical_area, scenario_data, default_agent, adv_agent)
            self.gpt_select_veh['critical_area'] = str(critical_area)
            self.gpt_select_veh['adv_agent'] = str(adv_agent)
            self.gpt_select_veh['min_ego_yaw'] = float(min_ego_yaw) if min_ego_yaw is not None else None
            self.gpt_select_veh['min_ego_traj'] = list(min_ego_traj.astype(float)) if min_ego_traj is not None else None
            self.gpt_select_veh['min_veh_traj'] = list(min_veh_traj.astype(float)) if min_veh_traj is not None else None
        ...
        for i, track_id in enumerate(tracks_ids):
            ...
            for j in range(0, 10):
                ...
                state_features['state/past/vel_yaw'][i][j] = np.arctan2(track_data['state']['velocity'][j][1],
                                                                        track_data['state']['velocity'][j][0])
            ...
            for j in range(10, 11):
                ...
                state_features['state/current/vel_yaw'][i] = np.arctan2(track_data['state']['velocity'][j][1],
                                                                        track_data['state']['velocity'][j][0])
            ...
            for j in range(11, 91):
                ...
                state_features['state/future/vel_yaw'][i][j - 11] = np.arctan2(track_data['state']['velocity'][j][1],
                                                                               track_data['state']['velocity'][j][0])
    ...
    def generate(self, mode='train'):
        ...
        with torch.no_grad():
            pred_trajectory, pred_score, _ = self.model(batch_data[0], self.device)

        # return traffic_motion_feat,pred_trajectory,trajs_AV
        if len(pred_trajectory) != 2:
            return traffic_motion_feat, self.adv_traj, trajs_AV, False
        ...
        for j, prob_OV in enumerate(probs_OV):
            ...
            for i, prob_AV in enumerate(probs_AV):
                P2 = prob_AV
                traj_AV = trajs_AV[i][::5]  # dowmsample to speed up
                ...
                for (Cx1, Cy1, yaw1, xA, yA, xB, yB, xC, yC, xD, yD), (
                    ...
                    if ego_adv_dist >= np.linalg.norm([0.5 * length_AV, 0.5 * width_AV]) + np.linalg.norm(
                        [0.5 * length_OV, 0.5 * width_OV]):
                        pass
                    elif Intersect([xA, yA, xB, yB], [xE, yE, xF, yF]) or Intersect([xA, yA, xB, yB],
                                                                                    [xF, yF, xG, yG]) or \
                        Intersect([xA, yA, xB, yB], [xG, yG, xH, yH]) or Intersect([xA, yA, xB, yB],
                                                                                   [xH, yH, xE, yE]) or \
                        Intersect([xB, yB, xC, yC], [xE, yE, xF, yF]) or Intersect([xB, yB, xC, yC],
                                                                                   [xF, yF, xG, yG]) or \
                        Intersect([xB, yB, xC, yC], [xG, yG, xH, yH]) or Intersect([xB, yB, xC, yC],
                                                                                   [xH, yH, xE, yE]) or \
                        Intersect([xC, yC, xD, yD], [xE, yE, xF, yF]) or Intersect([xC, yC, xD, yD],
                                                                                   [xF, yF, xG, yG]) or \
                        Intersect([xC, yC, xD, yD], [xG, yG, xH, yH]) or Intersect([xC, yC, xD, yD],
                                                                                   [xH, yH, xE, yE]) or \
                        Intersect([xD, yD, xA, yA], [xE, yE, xF, yF]) or Intersect([xD, yD, xA, yA],
                                                                                   [xF, yF, xG, yG]) or \
                        Intersect([xD, yD, xA, yA], [xG, yG, xH, yH]) or Intersect([xD, yD, xA, yA],
                                                                                   [xH, yH, xE, yE]):
                        P3 = uncertainty
                        break
